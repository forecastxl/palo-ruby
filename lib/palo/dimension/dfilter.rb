...
eos
      def execute(params = {})
        response = @session.query('/dimension/dfilter', params)
        puts response
...
eos
      def parse_line(values)
        keys = %w(element name_element position level indent depth type number_parents parents number_children children weights type exists value)
        result = Hash[keys.zip(values)]
        trim_quotes result, 'name_element', 'value'
        result
      end
    end
  end
